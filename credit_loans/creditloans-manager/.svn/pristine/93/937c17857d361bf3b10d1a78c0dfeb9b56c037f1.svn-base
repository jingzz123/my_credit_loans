		...
		<tr>
			<td align="right" style="width: 50%;border-top:0;">
				<select id="sel_all_area2"  multiple="multiple" size="10" style="width:100%;height:200px">
				   <c:forEach items="${baseActionNameDTOList}" var="actionname" varStatus="status">
			 ...
			 <td style="width: 50%;border-top:0;">
				 <div style="margin-bottom: 2px;">
				  ...
				  <span id="error-selectCondition" style="color: red">*</span>
							  ...
							  <input type="hidden" id="rulesetId" name="rulesetId" value="${rulesetId }">
							  <input type="hidden" id="oldName" name="oldName" value="">
							  <input type="hidden" id="oldSequence" name="oldSequence" value="">
							  <div class="form-group">
...
$().ready(function() {
	$("#ruleInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/rule/doAdd",
			    ...
			    error: function () {
			        alert("save regulation rule error");
			    }	
			});
		}
	});

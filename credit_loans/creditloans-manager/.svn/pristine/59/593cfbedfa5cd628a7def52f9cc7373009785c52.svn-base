...
$().ready(function() {
	$("#validatorElementInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/validatorelement/doAdd",
			    contentType: "application/json",
			    data: JSON.stringify(validatorElementInfoForm),
			    success: function (validatorElementId) {
			    	if(validatorElementId>0){
				        alert("添加validatorElementId成功！");
			    	}else{
			    		alert("添加validatorElementId失败！");
			    		
			    	}
			    	window.location.href = "${pageContext.request.contextPath}/regulation/basepackage/validatorelement/showList";
			    },
			    error: function () {
			        alert("save regulation validatorelement error");
			    }	
			});
		}
	});

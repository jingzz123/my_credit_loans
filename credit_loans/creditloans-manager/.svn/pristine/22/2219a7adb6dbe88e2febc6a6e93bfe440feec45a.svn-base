...
$().ready(function() {
	...
	$("#conditionInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/condition/doEdit",
			    contentType: "application/json",
			    data: JSON.stringify(conditionInfoForm),
			    success: function () {
			    	alert("修改用户成功！");
			    	window.location.href = "${pageContext.request.contextPath}/regulation/basepackage/condition/showList";
			    },
			    error: function () {
			        alert("edit regulation condition error");
			    }	
			});
		}
	});
...
function loadSelect(){	
	var operator = "${rulesetConditionDTO.operator}";
	var type = "${rulesetConditionDTO.type}";
	
	
	$("#operator_select").find("option[value="+operator+"]").attr("selected",true);
	$("#type_select").find("option[value="+type+"]").attr("selected",true);
}
function goback(){
	window.location.href = "${pageContext.request.contextPath}/regulation/basepackage/condition/showList?rulesetId=${rulesetConditionDTO.rulesetId}";
}
</script>
				    	...
				    	<fieldset>
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      <input type="text" id="name" name="name" class="form-control" placeholder="输入条件名称" 
							       value="${rulesetConditionDTO.name }">
							    </div>
							   ...
							   <label for="description" class="col-sm-1 control-label">条件描述</label>
							    <div class="col-sm-3">
							      ...
							      value="${rulesetConditionDTO.description }">
							    </div>
							  ...
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      ...
							      value="${rulesetConditionDTO.sequence }">
							    </div>
							    <span id="error_sequence" style="color: red" class="col-sm-2"></span>
							</div>
							</fieldset>
							  <input type="hidden" id="rulesetId" name="rulesetId" value="${rulesetConditionDTO.rulesetId }">
							  <input type="hidden" id="id" name="id" value="${rulesetConditionDTO.id }">
							  <input type="hidden" id="oldSequence" name="oldSequence" value="${rulesetConditionDTO.oldSequence}">
							  <input type="hidden" id="oldName" name="oldName" value="${rulesetConditionDTO.oldName}">
							  <div class="form-group">

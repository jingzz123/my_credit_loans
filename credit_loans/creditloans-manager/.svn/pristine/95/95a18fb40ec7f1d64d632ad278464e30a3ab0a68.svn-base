...
$().ready(function() {
	...
	$("#variableSimpleInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/variablesimple/doEdit",
			    ...
			    error: function () {
			        alert("edit regulation variablesimple error");
			    }	
			});
		}
	});
...
function loadSelect() {
	var type = "${baseVariableSimpleDTO.type}";
				    	...
				    	<fieldset>
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      <input type="text" id="name" name="name" class="form-control" placeholder="输入简单变量名称" 
							       value="${baseVariableSimpleDTO.name }">
							    </div>
							   ...
							   <label for="description" class="col-sm-1 control-label">简单变量描述</label>
							    <div class="col-sm-3">
							      ...
							      value="${baseVariableSimpleDTO.description }">
							    </div>
							  ...
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      ...
							      value="${baseVariableSimpleDTO.initValue }">
							    </div>
							  ...
							  <div class="form-group">
							    ...
							    <div id="base_showspace" class="col-sm-3">
							    	<c:forEach items="${baseVariableSimpleDTO.baseElementDTOList}" var="base">
							    		<input type="checkbox"  value="${base.id}" name="baseIdList" <c:if test="${base.variableSimpleFlag==1 }">checked</c:if> />${base.name}
							    	</c:forEach>
							    </div>
							    <span id="error_baseIdList" style="color: red" class="col-sm-2"></span>
							  </div> 
							</fieldset>
							  <input type="hidden" id="oldName" name="oldName" value="${baseVariableSimpleDTO.oldName}">
							 <input type="hidden" name="id" value="${baseVariableSimpleDTO.id}">
							  <div class="form-group">

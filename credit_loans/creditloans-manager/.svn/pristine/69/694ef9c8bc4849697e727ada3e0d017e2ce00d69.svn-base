...
$().ready(function() {
	$("#elementInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/element/doEdit",
			    ...
			    error: function () {
			        alert("edit regulation element error");
			    }	
			});
		}
	});
			    	...
			    	<div class="col-md-12">
				    	...
				    	<fieldset>
							  <div class="form-group">
							   ...
							   <label for="description" class="col-sm-1 control-label">包描述</label>
							    <div class="col-sm-3">
							      ...
							      value="${baseElementDTO.description }">
							    </div>
							  ...
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">							    
							    	<select id="schemaId_select" name="schemaId" class="form-control">
							    		...
							    		<option value="${schema.id}" <c:if test="${schema.id == baseElementDTO.schemaId }">selected</c:if>>${schema.name}</option>
							    		</c:forEach>
							    	</select>
							  	</div>
							    ...
							    <div class="col-sm-3">							    
							    	<select id="departmentId_select" name="departmentId" class="form-control">
							    		...
							    		<option value="${dep.id}" <c:if test="${dep.id == baseElementDTO.departmentId }">selected</c:if>>${dep.name}</option>
							    		</c:forEach>
							    	</select>
							  	</div>
							  ...
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      <input id="isInner_yes" name="isInner" type="radio" value="Y" <c:if test="${baseElementDTO.isInner == 'Y' }">checked</c:if>/>是 
							      <input id="isInner_not" name="isInner" type="radio" value="N" <c:if test="${baseElementDTO.isInner == 'N' }">checked</c:if>/>否
							    </div>
							    <span id="error_isInner" style="color: red" class="col-sm-2"></span>
							  	<div class="form-group">
							  	<label for="isUsed" class="col-sm-1 control-label">是否可用</label>
							    <div class="col-sm-3">
							      <input id="isUsed_yes" name="isUsed" type="radio" <c:if test="${baseElementDTO.isUsed == 'Y' }">checked</c:if> value="Y"/>是 
							      <input id="isUsed_not" name="isUsed" type="radio" <c:if test="${baseElementDTO.isUsed == 'N' }">checked</c:if> value="N"/>否
							    </div>
							 ...
							 <input type="hidden" name="id" value="${baseElementDTO.id}">
							  <div class="form-group">

...
$().ready(function() {
	$("#variableTransactionInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/variabletransaction/doEdit",
			    ...
			    error: function () {
			        alert("edit regulation variabletransaction error");
			    }	
			});
		}
	});
				    	...
				    	<fieldset>
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      <input type="text" id="name" name="name" class="form-control" placeholder="输入业务变量名称" 
							       value="${baseVariableTransactionDTO.name }">
							    </div>
							   ...
							   <label for="description" class="col-sm-1 control-label">业务变量描述</label>
							    <div class="col-sm-3">
							      ...
							      value="${baseVariableTransactionDTO.description }">
							    </div>
							   ...
							   <fieldset>
								...
								<div class="col-sm-3">
									<select class="form-control" id="fieldId_select" name="fieldId" >
							    		...
							    		<option value="${field.id}" <c:if test="${field.id == baseVariableTransactionDTO.fieldId }">selected</c:if> >${field.name}</option>
							    		</c:forEach>
									</select>
								</div>
							  ...
							  <div class="form-group">
							    ...
							    <div id="base_showspace" class="col-sm-3">
							    	<c:forEach items="${baseVariableTransactionDTO.baseElementDTOList}" var="base">
							    		<input type="checkbox"  value="${base.id}" name="baseIdList" <c:if test="${base.variableTransactionFlag==1 }">checked</c:if> />${base.name}
							    	</c:forEach>
							    </div>
							    <span id="error_baseIdList" style="color: red" class="col-sm-2"></span>
							  </div> 
							 </fieldset>
							 <input type="hidden" id="oldName" name="oldName" value="${baseVariableTransactionDTO.oldName}">
							 <input type="hidden" name="id" value="${baseVariableTransactionDTO.id}">
							  <div class="form-group">

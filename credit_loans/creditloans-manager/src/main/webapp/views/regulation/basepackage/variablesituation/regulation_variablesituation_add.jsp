...
$().ready(function() {
	$("#variableSituationInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/variablesituation/doAdd",
			    contentType: "application/json",
			    data: JSON.stringify(variableSituationInfoForm),
			    success: function (variableSituationId) {
			    	if(variableSituationId>0){
				        alert("添加variableSituation成功！");
			    	}else{
			    		alert("添加variableSituation失败！");
			    		
			    	}
			    	window.location.href = "${pageContext.request.contextPath}/regulation/basepackage/variablesituation/showList";
			    },
			    error: function () {
			        alert("save regulation variablesituation error");
			    }	
			});
		}
	});

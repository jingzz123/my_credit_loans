...
$().ready(function() {
	...
	$("#variableExpressionInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/regulation/basepackage/variableexpression/doEdit",
			    ...
			    error: function () {
			        alert("edit regulation variableexpression error");
			    }	
			});
		}
	});
...
function loadSelect() {
	var type = "${baseVariableExpressionDTO.type }";
				    	...
				    	<fieldset>
							  <div class="form-group">
							    ...
							    <div class="col-sm-3">
							      <input type="text" id="name" name="name" class="form-control" placeholder="输入表达式变量名称" 
							       value="${baseVariableExpressionDTO.name }">
							    </div>
							   ...
							   <label for="description" class="col-sm-1 control-label">表达式变量描述</label>
							    <div class="col-sm-3">
							      <input type="text" id="description" name="description" class="form-control" placeholder="输入表达式量描述"
							       value="${baseVariableExpressionDTO.description }">
							    </div>
							   ...
							   <label for="content" class="col-sm-1 control-label">表达式</label>
							    <div class="col-sm-3">
							      <input type="text" id="content" name="content" class="form-control" placeholder="输入表达式"
							       value="${baseVariableExpressionDTO.content }">
							    </div>
							  ...
							  <div class="form-group">
							    ...
							    <div id="base_showspace" class="col-sm-3">
							    	<c:forEach items="${baseVariableExpressionDTO.baseElementDTOList}" var="base">
							    		<input type="checkbox"  value="${base.id}" name="baseIdList" <c:if test="${base.variableExpressionFlag==1 }">checked</c:if> />${base.name}
							    	</c:forEach>
							    </div>
							    <span id="error_baseIdList" style="color: red" class="col-sm-2"></span>
							  </div> 
							</fieldset>
							<input type="hidden" id="oldName" name="oldName" value="${baseVariableExpressionDTO.oldName}">
							<input type="hidden" name="id" value="${baseVariableExpressionDTO.id }">
							  <div class="form-group">

...
$(document).ready(function(){
	...
	if(activeId==null||activeId==""){
		activeId = "fmenu_${platformuser.ownerPlatformMenuDtoList[0].id}";
	}
	...
	function loadSecMeun(activeId,activeSecMenuId){
		...
		var str = "";
		<c:forEach items="${platformuser.ownerPlatformMenuDtoList}" var="secMenu">
			if("${secMenu.id}"==fid){
		...
		if(activeSecMenuId==null||activeSecMenuId==""){
			...
			$.each($("#secMenu_show li a"), function(i,val){
				if(url==val.href){
					...
					$.cookie("activeSecMenuId", $("#"+urlId).parent().attr("id"), { expires: 1 , path:"/creditloans-manager/" });
				}
			});
		}else{
	...
	function activeMenu(obj){
		$.cookie("activeId", obj.id, { expires: 1 , path:"/creditloans-manager/" });
		$.cookie("activeSecMenuId", '', { expires: -1 , path:"/creditloans-manager/" });
	}
	
	function activeSecMenu(id){
		$.cookie("activeSecMenuId", id, { expires: 1 , path:"/creditloans-manager/" });
	}
</script>
<body>
<div class="navbar navbar-default" id="navbar">
	<div class="navbar-container" id="navbar-container">
		<div class="navbar-header pull-left"> <a href="" class="navbar-brand"> <small>征信报送云平台-后台管理 </small> </a><!-- /.brand --> 
		</div>
		<!-- /.navbar-header -->
		<div class="navbar-header pull-left" role="navigation">
			<ul class="nav abc-nav abc-nav-next">
				<c:forEach items="${platformuser.ownerPlatformMenuDtoList}" var="fmenu">
					<c:if test="${fmenu.fid==0}">
						<li class="light-blue" style="width: 115px"> <a href="${pageContext.request.contextPath}${fmenu.url}" id="fmenu_${fmenu.id}" onclick="activeMenu(this)"> <i class="${fmenu.icon }"></i><span>${fmenu.name }</span></a></li>
					</c:if>
				</c:forEach>
			</ul>
			<!-- /.abc-nav-next --> 
		...
		<div class="navbar-header pull-right" role="navigation">
			<ul class="nav abc-nav">
				<li class="light-blue"> <a data-toggle="dropdown" href="#" class="dropdown-toggle"> <img class="nav-user-photo" src="${pageContext.request.contextPath}/static/images/img/user.jpg" alt="Jason's Photo" /> <span class="user-info"> <small>欢迎你，</small> ${platformuser.name } </span> <i class="icon-caret-down"></i> </a>

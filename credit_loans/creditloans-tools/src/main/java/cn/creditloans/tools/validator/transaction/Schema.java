...
public class Schema implements Serializable{
	private static final long serialVersionUID = 6949532670316623224L;
    ...
    /*The following is saved with schema attributes, add by small 2004.4.15
     * If the schema is "NestSchema", there must be two key value:
     *  =>nested=yes; id_length={the Id Length of subSchema}
     * If the schema type="Various_Length", there must be one key value: delimiter=
     * If the schema is "NestSchema" && its shema and its subSchemas have "Various_Length" type, there must be one key value:
     *  =>field_delimiter= {the delimiter of all "Various_Length" Schemas and field_delimiter must be the same}
     */
     ...
     * @param columnLengths column lengths
     */
     ...
     * @param columnNames column names
     * @param columnTypes column types
     * @param delimeter delimeter to separator the column value. It is used for the variable length of
     * record
     */
    ...
    /**
     * @return type of the Transaction this schema defined. It can be either VARIOUS_LENGTH or FIXED_LENGTH
     */
    public int getType() {
        return m_type;
    }
    ...
    /**
     * @return delimeter used if the data record represented by the schema has variable length.
     */
    ...
    /*
     *@param recordDef The Schema object to compared with this Schema
     *return true if this recordDef's  is a subset of the parameter recordDef
     */
    public boolean isCompatible1(Schema def) {
        int[] types = def.getColumnTypes();
        int[] lengths = def.getColumnLengths();
        for(int i = 0; i < m_columnNames.length; i++) {
            if(m_columnTypes[i] != types[i] || m_columnLengths[i] != lengths[i])
                return false;
        }
        return true;
    }
    /*
     *@param recordDef The Schema object to compared with this Schema
     *return true if this recordDef is a subset of the parameter recordDef
     */

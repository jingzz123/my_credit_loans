...
import com.intellicredit.platform.util.Tokenizer;

public class NestTransaction extends Transaction {

	private static final long serialVersionUID = 3478925547128890551L;
	...
	private int[] m_subsegmentLengths;// It will be used when the type of the
										// schema and its subSchemas are
										// "Fixed_Length" and has no delimiter
	private int m_baseSegmentLength;
	// add by small 2004.4.15
	private String m_fieldDelimiter = null;// It will be used when all columns
											// are seperated by same delimiter
	 ...
	 * 
	 * @param schema
	 *            netSchema used by the Transaction
	 */
	...
	public NestTransaction(NestSchema schema) {
		...
		m_idLength = Integer.parseInt(getSchema().getProperty(
				TranConstants.ID_LENGTH));
		m_fieldDelimiter = getSchema().getProperty(
				TranConstants.FIELD_DELIMITER);
	...
	public Transaction[] getSubTransaction(int index) {
		...
		if (m_nestsubtransactions[index].size() == 0)
			...
			return (Transaction[]) m_nestsubtransactions[index]
	...
	/**
	 * set transaction data for fixed length transaction and data has no
	 * delimiter
	 * 
	 * @param data
	 *            transaction data
	...
	/**
	 * set transaction data that are delimited by del no matter the schema is
	 * various length or fixed length.
	 * 
	 * @param data
	 *            transaction data
	 * @return error code, 0 means no error
		...
		if (segmentId == null || segmentId.length() == 0) {
			...
			for (int i = 0; i < m_subschemas.length; i++) {
				if (m_subschemas[i].m_columnNames[0].equals(segmentId)) {
					...
					Transaction[] subtransactions = (Transaction[]) m_nestsubtransactions[i]
							.toArray(new Transaction[0]);

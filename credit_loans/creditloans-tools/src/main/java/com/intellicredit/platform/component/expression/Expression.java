...
public class Expression {
     ...
     */
    public Expression(boolean traverse_in,
    ...
    public void addStandardFunctions() {
        ...
        funTab.put("substr", new SubStr());
        funTab.put("d_curtime", new DCurTime());
        ...
        funTab.put("avg", new Avg());
        funTab.put("get", new Get());
        funTab.put("curtime", new CurTime());
        funTab.put("rand", new com.intellicredit.platform.component.expression.function.Random());
    }
     ...
     * contained in the expression.
     * @param functionName The name of the function
     * @param functionClassName : for example: com.intellicredit.Test
     * Add by wu 2004/10/13
     */
    public void addFunction(String functionName, String funClassName) {
        if (functionName == null || functionName.trim().length() == 0
            || funClassName == null || funClassName.trim().length() == 0) {
            return;
        }
        Class<?> superClass = null;
        Object obj = null;
        try {
            superClass = Class.forName("com.intellicredit.platform.component.expression.function.PostfixMathCommand");
            Class<?> funClass = Class.forName(funClassName);
            Class<?> curSuperClass = funClass.getSuperclass();
            if (curSuperClass == superClass) {
                obj = funClass.newInstance();
            }
        }
        catch (Exception e) {
            e.printStackTrace();
        ...
        if (obj != null) {
            if (!funTab.contains(functionName)) { //如果存在相同key,那不将当前加入
                //System.out.println("add to the function!");
                //System.out.println("functionName="+functionName);
                funTab.put(functionName, obj);
            }
        }
        else {

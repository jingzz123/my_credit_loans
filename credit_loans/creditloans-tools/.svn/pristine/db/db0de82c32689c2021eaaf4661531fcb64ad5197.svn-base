...
public class ClassUtil {
	...
	private static final Object[][] _primitives = {
			{ "boolean", boolean.class, "B" }, { "byte", byte.class, "Y" },
			{ "char", char.class, "C" }, { "double", double.class, "D" },
			{ "float", float.class, "F" }, { "int", int.class, "I" },
			{ "long", long.class, "L" }, { "short", short.class, "S" },
			{ "void", void.class, null } };
	private static final Map<Object, Object> _primitiveArrayTypeNames;
	private static final Map<Object, Object> _primitiveTypes;
	public static final Comparator<Object> propertyDescriptorNameComparator;
	...
	static {
		...
		_primitiveArrayTypeNames = Collections
				.unmodifiableMap(new HashMap<Object, Object>() {
					private static final long serialVersionUID = -38949282440582546L;
		...
		_primitiveTypes = Collections
				.unmodifiableMap(new HashMap<Object, Object>() {

					private static final long serialVersionUID = -8392041628428961200L;
		...
		propertyDescriptorNameComparator = new Comparator<Object>() {
	...
	public static Class<?> classForName(String className)
			...
			for (int j = index, n = className.length(); j < n; j++) {
				...
				default:
					...
					if (Character.isWhitespace(ch))
						break;
					throw new IllegalArgumentException(className + INVALID_TYPE);
				}
			}
			if (lbracket) {
		...
		if (dimensions != 0) {
			javaClass = null;
			if (primitive) {
				className = (String) _primitiveArrayTypeNames.get(className);
			}
	...
	public static PropertyDescriptor[] introspect(Class<?> beanClass,
		...
		try {
			...
			if (!beanClass.isInterface() || stopClass == null) {
				...
				propDescs = props.toArray(new PropertyDescriptor[props.size()]);
			}
	...
	public static String[] namesForClasses(Class<?>[] parmTypes) {
		...
		for (int i = 0, dims = 0; i < parmTypes.length; i++, dims = 0) {
			...
			if (dims != 0) {
				StringBuffer sb = new StringBuffer(className.length() + 2
						* dims);
				sb.append(className);
	 ...
	 *            <code>true</code> to include the modifiers.
	 * @param includeReturn
	 *            <code>true</code> to include the return type.
	 * @param includeClass
	 *            <code>true</code> to include the class name.
	 * @param includeMethod
	 *            <code>true</code> to include the method name.
	 * @param includeParameterTypes
	 *            <code>true</code> to include the parameters.
	 * @param includeExceptions
	 *            <code>true</code> to include the exceptions.
	 * @return String representation of the method in JVM runtime format.
	 */
	public static String signatureFromMethod(Method method,
			boolean includeModifiers, boolean includeReturn,
			boolean includeClass, boolean includeMethod,
			boolean includeParameterTypes, boolean includeExceptions) {

...
$().ready(function() {
		$("#bfjnmxxxInfoForm").validate({
			...
			submitHandler:function(){
				...
				jQuery.ajax({
					type: "post",
				    url: "${pageContext.request.contextPath}/db/bfjnmxxx/doAdd",
				    contentType: "application/json",
				    data: JSON.stringify(bfjnmxxxInfoForm),
				    success: function (bfjnmxxxId) {
				    	if(bfjnmxxxId>0){
					        alert("添加成功！");
				    	}else{
				    		alert("添加失败！");
				    	...
				    	window.location.href = "${pageContext.request.contextPath}/db/bfjnmxxx/showList?dbhtxxId=${dbhtxxId}";
				    },
				    error: function () {
				        alert("save bfjnmxxx error");
				    }	
				});
			}
								...
								<div class="col-sm-3">
									<div class="input-group date form_date col-md-3 form-control"
										...
										<input class="form-control" id="sjrq" name="sjrq" type="text"
											placeholder="实缴日期" readonly="readonly"> <span
											class="input-group-addon"><span
											class="glyphicon glyphicon-calendar icon-calendar"></span></span> <span
											class="input-group-addon"><span
											class="glyphicon glyphicon-remove icon-remove"></span></span>

								...
								<div class="col-sm-3">
									<div class="input-group date form_date col-md-3 form-control"
										...
										<input class="form-control" id="csrq" name="csrq" type="text"
											placeholder="出生日期" readonly="readonly"> <span
								...
								<div class="col-sm-3">
									<div class="input-group date work_date col-md-3 form-control"
										...
										<input class="form-control" id="bdwgzqsnf" name="bdwgzqsnf" type="text"
											placeholder="单位工作起始年份" readonly="readonly"> <span
											class="input-group-addon"><span
											class="glyphicon glyphicon-calendar icon-calendar"></span></span> <span
											class="input-group-addon"><span
											class="glyphicon glyphicon-remove icon-remove"></span></span>
...
$().ready(function() {
	...
	$("#grjbxxInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/p2p/grjbxx/doAdd",
			    contentType: "application/json",
			    data: JSON.stringify(grjbxxInfoForm),
			    success: function (p2pGrjbxxId) {
			    	if(p2pGrjbxxId>0){
				        alert("添加成功！");
			    	}else{
			    		alert("添加失败！");
...
function checkSfxx(){
	if($('#sfxx').is(':checked') == true){
		$('#sfxx').attr('checked','checked');
		$('#sfxxd').removeAttr('disabled');
	}
	if($('#sfxx').is(':checked') == false){
		$('#sfxx').removeAttr('checked');
		$('#sfxxd').attr('disabled','disabled');
	}
}

					...
					<form id="tsjyxxInfoForm" class="form-horizontal">
						<fieldset>
								...
								<div class="col-sm-3">
									<div class="input-group date form_date col-md-3 form-control"
										...
										<input class="form-control" id="fsrq" name="fsrq" type="text"
											placeholder="发生日期" readonly="readonly" onchange="removeZjhm();"> <span
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<textarea id="mxxx" name="mxxx" class="form-control" style="resize:none;" rows="5" cols="20"></textarea>
								</div>
...
$().ready(function() {
	...
	$("#tsjyxxInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/p2p/tsjyxx/doAdd",
			    contentType: "application/json",
			    data: JSON.stringify(tsjyxxInfoForm),
			    success: function (p2pTsjyxxId) {
			    	if(p2pTsjyxxId>0){
				        alert("添加成功！");
			    	}else{
			    		alert("添加失败！");

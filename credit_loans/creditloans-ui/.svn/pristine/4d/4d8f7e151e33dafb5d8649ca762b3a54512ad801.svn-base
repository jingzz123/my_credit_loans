...
$().ready(function() {
	$("#dcmxxxInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/db/dcmxxx/doEdit",
			    contentType: "application/json",
			    data: JSON.stringify(dcmxxxInfoForm),
			    success: function () {
			    	alert("修改成功！");
			    	window.location.href = "${pageContext.request.contextPath}/db/dcmxxx/showList?dbhtxxId=${dbDcmxxxDTO.dbhtxxId}";
			    },
			    error: function () {
			        alert("edit dcmxxx error");
			    }	
			});
		}
	});
					...
					<form id="dcmxxxInfoForm" class="form-horizontal">
						<fieldset>
							...
							<div class="col-sm-3">
								<div class="input-group date form_date col-md-3 form-control"
									...
									<input class="form-control" id="dcrq" name="dcrq" type="text"
										placeholder="代偿日期" readonly="readonly" value="${dbDcmxxxDTO.dcrq}"> <span
							...
							<div class="col-sm-3">
								<input type="text" id="dcje" name="dcje" class="form-control"
									placeholder="输入代偿金额" value="${dbDcmxxxDTO.dcje}">
							</div>
						...
						<fieldset>
							...
							<div class="form-group">
							    ...
							    <div class="col-sm-3">
							      	<c:if test="${dbDcmxxxDTO.status == 0}">
								    ...
								    <c:if test="${dbDcmxxxDTO.status == 1}">
						...
						<input type="hidden" name="orgCode" value="${dbDcmxxxDTO.orgCode}">
						<input type="hidden" name="dbhtxxId" value="${dbDcmxxxDTO.dbhtxxId}">
						<input type="hidden" name="id" value="${dbDcmxxxDTO.id}">
						<div class="form-group">

...
$().ready(function() {
	...
	$("#zqrjzhtxxInfoForm").validate({
		...
		submitHandler:function(){
			...
			jQuery.ajax({
				type: "post",
			    url: "${pageContext.request.contextPath}/db/zqrjzhtxx/doEdit",
			    contentType: "application/json",
			    data: JSON.stringify(zqrjzhtxxInfoForm),
			    success: function () {
			    	alert("修改成功！");
			    	window.location.href = "${pageContext.request.contextPath}/db/zqrjzhtxx/showList?dbhtxxId=${dbZqrjzhtxxDTO.dbhtxxId}";
			    },
			    error: function () {
			        alert("edit zqrjzhtxx error");
			    }	
			});
		}
	});
...
//临时处理，后面不用这种方式
function loadSelect(){	
	var zqrlx = "${dbZqrjzhtxxDTO.zqrlx}";
	var zqrzjlx = "${dbZqrjzhtxxDTO.zqrzjlx}";
	var ztw = "${dbZqrjzhtxxDTO.ztw}";
	
	$("#zqrlx_select").find("option[value="+zqrlx+"]").attr("selected",true);
	$("#zqrzjlx_select").find("option[value="+zqrzjlx+"]").attr("selected",true);
					...
					<form id="zqrjzhtxxInfoForm" class="form-horizontal">
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<input type="text" id="zqrmc" name="zqrmc" class="form-control"
										placeholder="债权人名称" value="${dbZqrjzhtxxDTO.zqrmc }">
								</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<input type="text" id="zqrzjhm " name="zqrzjhm" class="form-control"
										placeholder="债权人证件号码" value="${dbZqrjzhtxxDTO.zqrmc }">
								</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<input type="text" id="tx" name="tx" class="form-control"
										placeholder="投向" value="${dbZqrjzhtxxDTO.tx }">
								</div>
						...
						<fieldset>
							...
							<div class="form-group">
							    ...
							    <div class="col-sm-3">
							      	<c:if test="${dbZqrjzhtxxDTO.status == 0}">
								    ...
								    <c:if test="${dbZqrjzhtxxDTO.status == 1}">
						...
						<input type="hidden" name="orgCode" value="${dbZqrjzhtxxDTO.orgCode}">
						<input type="hidden" name="dbhtxxId" value="${dbZqrjzhtxxDTO.dbhtxxId}">
						<input type="hidden" name="id" value="${dbZqrjzhtxxDTO.id}">
						<div class="form-group">
